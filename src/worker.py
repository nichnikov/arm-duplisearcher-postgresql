...
def resulting_report(searched_data, result_tuples, found_data_l: list[Data], locale: str):
    ...
    searched_answers_moduls = {item.answerId: item.moduleId for item in searched_data}

    similarity_items = [
        ResultItem(
            searched_dict[sq_i].answerId,
            searched_dict[sq_i].cluster,
            searched_dict[sq_i].queryId,
...
class Worker:
    ...
    def delete(self, data: list[Data]) -> None:
        ...
        searched_data = self.text_storage.search_answers(ids=list(set(data_t.answer_ids)))
        q_ids = [x[2] for x in searched_data]
        q_ids = list(set(q_ids))
        self.matrix_list.delete(q_ids)
        self.text_storage.delete_queries(q_ids)
    ...
    def update(self, data: list[Data]) -> None:
        data_t = transpose(data)
        searched_data = self.text_storage.search_answers(ids=data_t.answer_ids)
